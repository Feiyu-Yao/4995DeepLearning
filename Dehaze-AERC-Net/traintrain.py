...
def train(net, trainset, valset, optim, criterion):
	...
	if True:
		ckp = torch.load('./trained_models/'+'its_train_cdnet_test')
	...
	for step in range(start_step+1, steps+1):
		...
		train_use=train_list[0:16]
		clean=torch.zeros([0,3,1848,752])
		dirty=torch.zeros([0,3,1848,752])
		for i in train_use:
			image_input_clean=Image.open('./trainset/clear_img'+'/'+i)
			image_input_dirty=Image.open('./trainset/hazy_image'+'/'+i)
		...
		if step % opt.eval_step == 0:
			...
			with torch.no_grad():
				ssim_eval, psnr_eval = val(net, 'valset', max_psnr, max_ssim, step)
